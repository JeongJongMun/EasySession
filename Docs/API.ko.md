...
**플러그인이 처음이신가요?** [Quick Start](QuickStart.md)가 노드 세 개로 호스트와 참가자를
...
| **Join Easy Session** | `SearchResult`, `bTravelOnSuccess=true`, `Password`, `AdditionalTravelOptions` | `JoinSession` 호출 후 호스트 주소를 해석하고 그 호스트로 이동합니다. 성공은 주소가 해석됐다는 뜻이지 호스트가 들여보냈다는 뜻이 아닙니다. 비밀번호나 난입 거절은 나중에 `Rejected` 디스커넥트로 도착합니다 ([가이드](Guide-Sessions.md)) |
...
| **Quick Match Easy Session** | `QuickMatchParams`, `PolicyClass`(선택) | 검색하고, 가장 좋은 결과에 참가하고, 없으면 직접 만듭니다. 위 세 노드를 대신 돌려주는 노드입니다 ([가이드](Guide-QuickMatch.md)) |
...
| Is Easy Matchmaking | `IsMatchmaking` | Quick Match가 돌고 있는가 |
...
> `Get Queue Status Description`은 작업 큐를, `Is Easy Matchmaking`,
> `Get Easy Matchmaking State`, `Get Online Subsystem Name`, `Is Online Subsystem
