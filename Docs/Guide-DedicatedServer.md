...
For a dedicated-only game, run Quick Match with **`Allow Host Fallback = false`** - clients will never accidentally become listen hosts, and an empty server list fails cleanly with `NoSessionsFound`.
