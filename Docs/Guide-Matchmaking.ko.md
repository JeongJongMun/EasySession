...
방식으로 광고됩니다. 폴백이 여는 방은 방금 한 검색으로도 찾을 수 있는 방입니다. 그래서 Host의
`Password`, `Hidden`, `Should Advertise`는 폴백 방에 적용되지 않습니다. 폴백은 항상 공개 방을
엽니다.

### 특정 방 하나를 매치메이킹하기

`Find Easy Sessions`가 받는 대상 질의가 여기서도 그대로 통합니다. `Search > Join Code`를
넣으면(C++에서는 친구, 소유자도) 패스가 그 방 하나를 사냥합니다. 숨긴 세션도
...
- **비밀번호가 걸린 세션** - `Join Password`를 주지 않으면 건너뜁니다.
