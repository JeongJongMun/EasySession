...
## "Can I cancel a Quick Play in progress?"

Yes: `Cancel Matchmaking` on the subsystem. The Quick Play node fires `OnFailure` with `Canceled`. In-flight online operations finish first (they cannot be aborted mid-call), so cancellation may take a moment.
