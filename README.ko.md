...
- **예제 캐릭터는 엔진 에셋을 빌려 씁니다.** 자체 메시를 포함하는 대신 엔진의 튜토리얼 마네킹(`/Engine/Tutorial/...`)을 사용해 플러그인 용량을 줄였습니다. 그 외 예제는 `/Engine/BasicShapes`와 `/Engine/MapTemplates`에만 의존합니다. 예제를 기반으로 개발한다면 캐릭터는 직접 교체하세요.
